...
workflow READ_ALIGNMENT_RNA {
    ...
    ch_merge_inputs = WorkflowFastq2bam.restoreMeta(ch_bams_united_sorted.runnable, ch_inputs)
        .map { meta, bams ->
            def meta_merge = [
                key: meta.group_id,
                id: meta.group_id,
    ...
    ch_markdups_inputs = Channel.empty()
        .mix(
            WorkflowFastq2bam.restoreMeta(SAMBAMBA_MERGE.out.bam, ch_inputs),
            WorkflowFastq2bam.restoreMeta(ch_bams_united_sorted.skip, ch_inputs),
        )
        .map { meta, bam ->
            def meta_markdups = [
                key: meta.group_id,
    ...
    ch_bams_ready = WorkflowFastq2bam.groupByMeta(
        WorkflowFastq2bam.restoreMeta(GATK4_MARKDUPLICATES.out.bam, ch_inputs),
        WorkflowFastq2bam.restoreMeta(GATK4_MARKDUPLICATES.out.bai, ch_inputs),
    )
